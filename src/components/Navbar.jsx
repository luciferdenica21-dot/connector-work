      ...
      <nav className="bg-[#0a0a0a]/90 backdrop-blur-md sticky top-0 z-50 text-white text-sm border-b border-blue-500/20 shadow-[0_4px_20px_rgba(0,0,0,0.5)]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-20">
            ...
            <div className="hidden md:flex absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 items-center space-x-12">
              <a href="#" className="relative font-medium tracking-widest hover:text-blue-400 transition-all group">{t('ГЛАВНАЯ')}<span className="absolute -bottom-2 left-0 w-0 h-[2px] bg-blue-500 transition-all group-hover:w-full"></span></a>
              
              <div className="relative group/dropdown">
                <button className="relative font-medium tracking-widest hover:text-blue-400 transition-all flex items-center gap-1 cursor-default">
                  {t('УСЛУГИ')}
                ...
                <div className="absolute left-0 mt-2 w-72 bg-[#0a0a0a] border border-blue-500/20 rounded-xl py-4 opacity-0 invisible group-hover/dropdown:opacity-100 group-hover/dropdown:visible transition-all duration-300 z-[60] shadow-2xl backdrop-blur-xl">
                  {servicesList.map((sKey) => (
                    <a key={sKey} href={`#services?service=${t(sKey)}`} className="block px-6 py-2 text-[10px] uppercase tracking-widest text-white/70 hover:text-blue-400 hover:bg-white/5 transition-colors">{t(sKey)}</a>
                  ))}
                </div>
              </div>
        ...
        <div className={`md:hidden overflow-hidden transition-all duration-500 ease-in-out ${isOpen ? 'max-h-[1000px] border-t border-blue-500/20' : 'max-h-0'}`} style={{ backgroundColor: isScrolling ? 'rgba(10, 10, 10, 0.5)' : 'rgba(10, 10, 10, 1)' }}>
          <div className="py-8 px-6 flex flex-col space-y-6">
            ...
            {/* Адаптированная кнопка входа для телефонов */}
            <button 
              onClick={() => { setIsAuthOpen(true); setIsOpen(false); }}
              className="w-full flex items-center justify-center gap-3 py-4 border border-white/10 bg-white/5 rounded-xl text-white hover:border-blue-500/50 transition-all active:scale-95"
            >
              ...
              <span className="text-[11px] font-bold uppercase tracking-[0.2em]">{t("ЛИЧНЫЙ КАБИНЕТ")}</span>
            </button>
            ...
            <button onClick={() => { setIsOrderOpen(true); setIsOpen(false); }} className="w-full bg-blue-500/20 border border-blue-500/50 text-blue-400 py-4 rounded-xl text-xs font-bold uppercase tracking-[0.2em] active:scale-95 transition-transform">{t("Заказать проект")}</button>
          </div>
        </div>
      </nav>
