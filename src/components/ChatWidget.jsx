      ...
      {isOpen && (
        <div className="absolute bottom-20 right-0 w-[320px] h-[450px] bg-[#0a0a0a] border border-blue-500/20 rounded-[2rem] shadow-2xl flex flex-col overflow-hidden backdrop-blur-xl animate-fadeIn">
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar">
            {messages.map((msg) => (
              <div key={msg._id || msg.id} className={`flex ${isUserMessage(msg) ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[80%] px-3 py-2 rounded-2xl text-xs ${isUserMessage(msg) ? 'bg-blue-600 text-white rounded-tr-none' : 'bg-white/10 text-white/80 rounded-tl-none'}`}>
                  <p>{msg.text}</p>
